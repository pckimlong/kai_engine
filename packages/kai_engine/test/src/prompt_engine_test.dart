...
void main() {
  group('ContextEngine', () {
    ...
    test('generates prompt with parallel builders', () async {
      ...
      ).thenAnswer((_) async => [CoreMessage.backgroundContext('Current date: 2023-01-01')].lock);
    ...
    test('generates prompt with sequential builders', () async {
      ...
      when(
        () => mockBuilder.build(any(), any(), any()),
      ).thenAnswer((_) async => [CoreMessage.backgroundContext('Processed history')].lock);
    ...
    test('executes sequential builders in order', () async {
      ...
      when(() => mockBuilder1.build(any(), any(), any())).thenAnswer((invocation) async {
        ...
        final previous = invocation.positionalArguments[2] as IList<CoreMessage>;
        return [...previous, CoreMessage.backgroundContext('Sequential 1')].lock;
      });

      when(() => mockBuilder2.build(any(), any(), any())).thenAnswer((invocation) async {
        callOrder.add('builder2');
        final previous = invocation.positionalArguments[2] as IList<CoreMessage>;
        return [...previous, CoreMessage.backgroundContext('Sequential 2')].lock;
      });
    ...
    test('processes parallel and sequential builders correctly', () async {
      ...
      when(
        () => mockParallelBuilder.build(any(), any(), any()),
      ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel context')].lock);

      when(
        () => mockSequentialBuilder.build(any(), any(), any()),
      ).thenAnswer((_) async => [CoreMessage.backgroundContext('Sequential context')].lock);
    ...
    test('maintains correct order of mixed template types', () async {
      ...
      when(() => mockSequentialBuilder1.build(any(), any(), any())).thenAnswer((invocation) async {
        final previous = invocation.positionalArguments[2] as IList<CoreMessage>;
        return [...previous, CoreMessage.backgroundContext('Sequential context 1')].lock;
      });

      when(() => mockSequentialBuilder2.build(any(), any(), any())).thenAnswer((invocation) async {
        final previous = invocation.positionalArguments[2] as IList<CoreMessage>;
        return [...previous, CoreMessage.backgroundContext('Sequential context 2')].lock;
      });
    ...
    group('Input Template Positioning Tests', () {
      test('preserves input template position when placed between parallel builders', () async {
        final mockBuilder1 = MockParallelContextBuilder();
        final mockBuilder2 = MockParallelContextBuilder();

        when(
          () => mockBuilder1.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel 1')].lock);

        when(
          () => mockBuilder2.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel 2')].lock);
        ...
        expect(result.prompts[2].content, 'Tell me a joke'); // User input at position 2
        expect(result.prompts[3].content, 'Parallel 2');
      });

      test('preserves input template position when placed between sequential builders', () async {
        final mockBuilder1 = MockSequentialContextBuilder();
        final mockBuilder2 = MockSequentialContextBuilder();

        when(
          () => mockBuilder1.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Sequential 1')].lock);

        when(
          () => mockBuilder2.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Sequential 2')].lock);
      ...
      test('preserves input template position when placed between mixed builder types', () async {
        final mockParallel = MockParallelContextBuilder();
        final mockSequential = MockSequentialContextBuilder();

        when(
          () => mockParallel.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel context')].lock);

        when(
          () => mockSequential.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Sequential context')].lock);
      ...
      test('preserves input template position at the beginning', () async {
        ...
        expect(result.prompts[0].content, 'Tell me a joke'); // User input at position 0
        expect(result.prompts[1].content, 'System prompt');
        expect(result.prompts[2].content, 'Parallel context');
      });

      test('preserves input template position in complex mixed template order', () async {
        final mockParallel1 = MockParallelContextBuilder();
        final mockParallel2 = MockParallelContextBuilder();
        final mockSequential = MockSequentialContextBuilder();

        when(
          () => mockParallel1.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel 1')].lock);

        when(
          () => mockParallel2.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Parallel 2')].lock);

        when(
          () => mockSequential.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Sequential')].lock);
        ...
        expect(result.prompts[3].content, 'Tell me a joke'); // User input at position 3
      ...
      test('handles multiple builders with same position correctly', () async {
        ...
        when(
          () => mockBuilder1.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('First')].lock);

        when(
          () => mockBuilder2.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Second')].lock);

        when(
          () => mockBuilder3.build(any(), any(), any()),
        ).thenAnswer((_) async => [CoreMessage.backgroundContext('Third')].lock);
        ...
        expect(result.prompts[1].content, 'Tell me a joke'); // User input at position 1
        expect(result.prompts[2].content, 'Second');
        expect(result.prompts[3].content, 'Middle system');
        expect(result.prompts[4].content, 'Third');
      });

      test('allows real user messages before input template (memory use case)', () async {
        ...
        final engine = TestContextEngine([
          const PromptTemplate.system('System prompt'),
          PromptTemplate.buildParallel(mockMemoryBuilder), // Real user messages before input
        ...
        expect(result.prompts[4].content, 'Tell me a joke'); // New user input at the end
        expect(result.userMessage.content, 'Tell me a joke');
      });
    });
  });
